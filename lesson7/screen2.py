...
class Polyline(Vector):
    ...
    def set_points(self, speeds):
        ...
        for p in range(len(self.p)):
            self.p[p] = self.p[p] + speeds[p]
            if self.p[p][0] > SCREEN_DIM[0] or self.p[p][0] < 0:
                speeds[p] = (- speeds[p][0], speeds[p][1])
            if self.p[p][1] > SCREEN_DIM[1] or self.p[p][1] < 0:
                speeds[p] = (speeds[p][0], -speeds[p][1])
# =======================================================================================
...
if __name__ == "__main__":
    ...
    while working:
        ...
        p=Knot(points)
        p.draw_points()
        p.draw_points(p.get_knot(steps), "line", 3, color)
        if not pause:
